# Winter.Z 的图像识别 ver1.1——零基础中文使用手册

本手册当前代码对应 **1.1.0B 威力加强版**。
...
| **1.1.0B 威力加强版** | 推荐超算 | `yolov8x.pt`、1000 epochs、832、自动 batch、workers 48、patience 200 |

只想在个人电脑上训练时，请下载 1.1.0A。需要在 60 核计算节点上追求验证精度时，请下载 1.1.0B。两版的检测操作相同。
...
推荐在仓库的 **Releases** 页面选择版本并下载完整压缩包：

- “Winter.Z的图像识别ver1.1(显卡友好版)”附带官方 `yolov8n.pt`。
- “Winter.Z的图像识别ver1.1(威力加强版)”附带官方 `yolov8x.pt`。

它们都是官方初始训练权重，不是训练完成的识别模型。也可以点击 **Code → Download ZIP** 下载源码，但源码压缩包不附带模型权重。
...
两个 Release 的默认设置对比如下：

| 参数 | 1.1.0A 显卡友好版 | 1.1.0B 威力加强版 | 作用与区别 |
| --- | --- | --- | --- |
| `model` | `yolov8n.pt` | `yolov8x.pt` | A 使用轻量模型；B 使用 YOLOv8 最大的标准检测模型，精度潜力更高 |
| `epochs` | 100 | 1000 | 最大训练轮数；实际可能被提前停止 |
| `imgsz` | 640 | 832 | B 为小目标保留更多输入特征，但计算量更高 |
| `batch` | 8 | -1（自动） | A 固定每批 8 张；B 根据单张 GPU 可用显存自动选择 |
| `device` | 0 | 0 | 默认使用第 1 张可用 NVIDIA GPU |
| `workers` | 0 | 48 | A 优先保证 Windows 稳定；B 为 60 核节点并行加载数据 |
| `seed` | 42 | 42 | 两版使用相同随机种子，便于复现和比较 |
| `patience` | 30 | 200 | 验证指标连续多少轮不再改善时提前停止 |
| `optimizer` | `auto` | `AdamW` | A 由 Ultralytics 自动选择；B 固定使用 AdamW |
| `lr0` | 0.01（`auto` 可能覆盖） | 0.001 | 初始学习率；B 使用更谨慎的小步长微调大模型 |
| `lrf` | 0.01 | 0.01 | 最终学习率相对于初始学习率的比例 |
| `cos_lr` | `false` | `true` | B 使用余弦学习率衰减，使后期调整更加细致 |
| `close_mosaic` | 10 | 30 | 最后若干轮关闭 Mosaic；B 留出更多轮适应真实图像分布 |
| `multi_scale` | 0.0（关闭） | 0.25 | B 在训练中改变输入尺度，增强不同目标尺寸的适应性 |
| `cache` | `false` | `ram` | B 把数据缓存到内存，减少计算节点等待磁盘读取 |
| 训练输出目录名 | `uav_tree_stone` | `uav_tree_stone_yolov8x_accurate` | 都位于项目的 `runs/detect/` 下 |
| 默认检测模型 | `runs/detect/uav_tree_stone/weights/best.pt` | 同 A | 两版不指定 `--model` 时都读取这个原始训练目录 |
| 默认待检测目录 | `待检测图片` | `待检测图片` | 两版不指定 `--source` 时都检测该文件夹 |
| 检测后打开结果 | 开启 | 开启 | 可用 `--no-open` 临时关闭 |

1.1.0A 完整包附带官方 `yolov8n.pt`，1.1.0B 完整包附带官方 `yolov8x.pt`。若使用 GitHub 源码 ZIP，当前 `main` 采用 B 版训练设置，第一次训练时 Ultralytics 会尝试联网下载 `yolov8x.pt`。B 版以验证集综合指标选择 `best.pt`，连续 200 轮没有改善时停止，因此“1000 轮”是上限，不保证一定跑满。

### 60 核超算使用提醒

- 60 个 CPU 核心主要加快数据读取和增强，不能代替 NVIDIA CUDA GPU；大模型速度与可用 batch 仍主要取决于 GPU 和显存。
- 默认 `workers: 48` 为系统和其他任务保留 12 核。如果集群限制进程数或内存不足，应按管理员要求降低 workers。
- 默认 `cache: ram` 会将数据缓存到内存；若节点内存不足，改为 `cache: false`。
- 若节点有多张 GPU，可以使用 `--device "0,1"` 等方式指定；具体 GPU 编号以集群调度系统分配结果为准。
- 自带图片均为 416×416，因此没有盲目把训练尺寸提高到 1280；默认 832 已是两倍尺度。真正提升精度仍应优先增加高分辨率真实航拍标注数据。
...
```text
runs/detect/uav_tree_stone_yolov8x_accurate/
```
...
```bat
python train.py --resume "runs\detect\uav_tree_stone_yolov8x_accurate\weights\last.pt"
```
...
默认 `batch: -1` 会自动估算单张 GPU 可用 batch。如果仍看到 `CUDA out of memory`，可以明确使用 batch 1：

```bat
python train.py --config config\project.yaml --batch 1
```

如果 `yolov8x.pt` 即使 batch 1 仍无法运行，应换用 1.1.0A 显卡友好版，或临时通过 `--model yolov8l.pt` 使用较小模型。
...
不要盲信指标，只有真实航拍图片测试效果稳定时，才说明模型有实际价值。
...
如果模型在其他目录，添加 `--model "实际的best.pt路径"`。未填写 `--source` 时，程序会检测“待检测图片”内的全部支持图片。
...
检测完成后，Windows 资源管理器会自动打开这一次的结果目录。如果在远程服务器或不希望自动打开，可在命令末尾添加 `--no-open`。

## 10. 临时指定其他图片或目录

使用 `--source` 可以临时指定单张图片或其他图片文件夹：

```bat
python predict_image.py --model "实际的best.pt路径" --source "实际图片或目录路径"
```
...
优先下载 1.1.0B Release 完整包，其中已经附带官方 `yolov8x.pt`。也可以手动下载官方权重并放到项目根目录，再运行：

```bat
python train.py --config config\project.yaml --model "yolov8x.pt"
```
...
模型不在默认位置时，添加 `--model "实际的best.pt路径"`；需要临时检测别处的图片时，再添加 `--source "实际的图片或目录路径"`。
