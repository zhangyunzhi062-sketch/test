...
这一命令中的 PyTorch 文件来自官方 CUDA 11.6 软件源，因此保留 `--extra-index-url`，不要删除。
...
```bat
cd /d "%USERPROFILE%\Desktop\ultralytics-main"
```
...
现有数据没有可靠的 `forest`（森林）标注。模型可以检测许多棵树，但“许多树”不能科学地等同于一个森林检测框，因此本项目不会虚构森林类别。以后获得带森林标注的数据后，再增加这个类别。
...
所有配置都使用项目相对路径。因此项目无论解压到哪个磁盘或用户目录，只要保持内部目录结构不变，就可以使用。
...
项目 `database` 中的三套原始数据来自 Roboflow，原始 `data.yaml` 标注的许可均为 CC BY 4.0：
