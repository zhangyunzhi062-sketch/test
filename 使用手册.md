...
| 参数 | 默认值 | 解释 |
| --- | --- | --- |
| 模型 | `yolov8x.pt` | YOLOv8 最大的标准检测模型，优先追求精度 |
| epochs | 1000 | 最多完整学习数据 1000 次 |
| imgsz | 832 | 兼顾小目标表达与自带 416×416 数据的有效信息 |
| batch | -1 | 让 Ultralytics 按单张 GPU 可用显存自动选择 batch |
| device | 0 | 使用第 1 张可用 NVIDIA 显卡 |
| workers | 48 | 供 60 核节点并行加载数据，并为系统保留部分核心 |
| patience | 200 | 连续 200 轮验证指标没有改善才提前停止 |
| optimizer | AdamW | 使用稳定的 AdamW 优化器 |
| cos_lr | true | 使用余弦学习率衰减 |
| multi_scale | 0.25 | 在训练中改变输入尺度，增强尺度适应能力 |
| cache | ram | 把数据缓存到内存以减少读取等待 |

1.1.0B 完整包在项目根目录附带官方 `yolov8x.pt`。若使用 GitHub 源码 ZIP，第一次训练时 Ultralytics 会尝试联网下载该权重。训练会以验证集综合指标选择 `best.pt`，并在 200 轮没有改善时停止；因此“1000 轮”是上限，不保证一定跑满。
