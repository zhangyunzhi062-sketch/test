...
推荐在仓库的 **Releases** 页面下载“Winter.Z的图像识别ver1.1(显卡友好版)”完整压缩包；该包附带官方 `yolov8n.pt` 初始权重。也可以点击 **Code → Download ZIP** 下载源码，但源码压缩包不附带模型权重。
...
| 参数 | 默认值 | 解释 |
| --- | --- | --- |
| 模型 | `yolov8n.pt` | 与 1.0.0A 相同的轻量 YOLOv8 检测模型 |
| epochs | 100 | 与 1.0.0A 相同，最多完整学习数据 100 次 |
| imgsz | 640 | 输入图片缩放到 640 像素 |
| batch | 8 | 每次送入显卡 8 张图片 |
| device | 0 | 使用第 1 张可用 NVIDIA 显卡 |
| workers | 0 | Windows 下最稳妥的数据加载方式 |
| patience | 30 | 连续 30 轮没有改善就提前停止 |

1.1.0A 的训练模型、轮数和收敛参数均保持 1.0.0A 的默认值。从 Release 完整包解压时，项目根目录已经附带官方 `yolov8n.pt`；若使用 GitHub 源码 ZIP，第一次训练时 Ultralytics 会尝试联网下载该权重。
...
优先下载 1.1.0A Release 完整包，其中已经附带官方 `yolov8n.pt`。也可以手动下载官方权重并放到项目根目录，再运行：
